...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!body.slug || !body.category) {
    return jsonResponse({ error: 'slug und category sind erforderlich.' }, 400);
  }
  ...
  const webhookUrl = `${n8nApiUrl.replace(/\/+$/, '')}/webhook/admin/delete`;
  ...
  try {
    n8nResponse = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({ slug: body.slug, category: body.category }),
    });
  } catch {
  ...
  if (!data.success) {
    return jsonResponse({ error: data.error || 'Löschen fehlgeschlagen.' }, 400);
  }
